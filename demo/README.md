...
Set both `AWS_STORAGE_BUCKET_NAME` and `AWS_LOCATION` to store uploaded media
in S3 instead. `AWS_LOCATION` must identify a prefix reserved for this demo;
the settings reject a bucket without a prefix. Set `AWS_REGION` when the
bucket is outside `us-east-1`.
