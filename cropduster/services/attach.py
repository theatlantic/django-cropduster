...
def _attach(instance, field_name, source, *, sizes, metadata, crops, sources,
    ...
    try:
        ...
        _render_sizes(
            ...
            skip_existing=skip_existing, files=files)
...
def _render_sizes(result, crops, *, rows, tmp, permissive, skip_existing,
    ...
    for size in result.sizes:
        candidates = list(Size.flatten([size]))
        kept = {
            candidate.name for candidate in candidates
            if (skip_existing and rows.get(candidate.name) is not None
                and image.storage.exists(image.get_image_path(candidate.name)))}

        thumb = rows.get(size.name)
        hint = framing[size.name]
        if size.name not in kept and hint is not None:
            computed = thumb_for_size(image, size, best_crop=hint)
            if computed is None:
                error = CropDusterResizeException(
                    "Image (%sx%s) is too small for size %s"
                    % (image.width, image.height, size.name))
                result.errors[size.name] = error
                if size.required and not permissive:
                    raise error
                continue
            thumb = _with_geometry(thumb, computed)

        for candidate in candidates:
            if candidate.name not in kept:
                files.capture(image.get_image_path(candidate.name, tmp=tmp))
        rendered = image.save_size(
            size, thumb, image=source, tmp=tmp, permissive=permissive,
            skip_existing=skip_existing, commit=False, errors=result.errors,
            rows=rows)
        result.thumbs.update(rendered)
        if tmp:
            for name in kept.intersection(rendered):
                files.capture(image.get_image_path(name, tmp=True))
                _copy_to_tmp(image, name)
...
def _with_geometry(row, computed):
    if row is None:
        return computed
    for attname in ('width', 'height', 'crop_x', 'crop_y', 'crop_w', 'crop_h'):
