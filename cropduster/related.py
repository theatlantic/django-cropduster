...
from django.db.models.fields.files import ImageFileDescriptor, ImageFieldFile, FieldFile
...
class GenericRelation(RelatedField, Field):
    ...
    def __init__(self, to, **kwargs):
        ...
        self.image_kwargs['upload_to'] = kwargs.pop('upload_to', CROPDUSTER_MEDIA_ROOT)
        self.image_kwargs['storage'] = kwargs.pop('storage', None)
        self.image_kwargs['width_field'] = kwargs.pop('width_field', None)
        self.image_kwargs['height_field'] = kwargs.pop('height_field', None)
        self.image_kwargs['max_length'] = kwargs.pop('max_length', 100)
    ...
    def contribute_to_class(self, cls, name):
        ...
        self.image_field = models.ImageField(editable=False,
            default='', blank=True, **self.image_kwargs)
        ### HACK: manually fix creation counter
        self.image_field.creation_counter = self.creation_counter
        cls.add_to_class('%s_image' % (self.name,), self.image_field)
...
class ReverseGenericRelatedObjectsDescriptor(object):
    ...
    def __set__(self, instance, value):
        ...
        manager.clear()
        image_field_name = '%s_image' % self.field.name
        if value is None:
            setattr(instance, image_field_name, self.field.image_field)
            return self
        for obj in value:
            setattr(instance, image_field_name, obj.path)
            manager.add(obj)
        return manager
...
def create_generic_related_manager(superclass):
    ...
    class GenericRelatedObjectManager(superclass):
        def __init__(self, model=None, core_filters=None, instance=None,
            ...
            self.image_field_name = '%s_image' % self._field.name
