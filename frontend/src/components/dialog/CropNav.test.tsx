...
describe("moving between sizes", () => {
  ...
  it("marks a saved crop when its geometry becomes dirty", async () => {
    const { shadow, view } = await mountDialog({
      sizes: [TWO_SIZES[0]!],
      image: IMAGE,
      crops: { main: { x: 15, y: 0, w: 1270, h: 1016 } },
      cropThumbs: {
    ...
    const childWrapper = shadow.querySelector<HTMLElement>(
    ...
    const box = view.CropDusterDialog?.state.crops.main?.box;
    expect(box).toEqual({ x: 17, y: 0, w: 1270, h: 1016 });
