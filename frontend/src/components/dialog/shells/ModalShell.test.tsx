...
function widget(options: { prefix?: string } = {}): Promise<MountedFixture> {
  return mountWidget({
    ...
    sizes: HEADSHOT_SIZES,
    thumbs: [
      { id: 1, name: "main", width: 220, height: 180, tmp: false },
      { id: 2, name: "thumb", width: 110, height: 90, tmp: false },
    ],
  ...
  it("hydrates from the formset in one round trip", async () => {
    ...
    fixture.container
      .querySelector<HTMLOptionElement>(
        '[name="lead_image-0-thumbs"] option[value="2"]',
      )
      ?.setAttribute("data-tmp-file", "true");
    ...
    expect(body.get("tmp_thumbs")).toBe("2");
