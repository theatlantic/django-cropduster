...
import type { CropEntry, DialogState, SourceImage } from "./types";
...
function enterSize(model: DialogModel, index: number): DialogModel {
  ...
  const box =
    ...
    defaultCropBox(source.width, source.height, size);
  return {
    ...next,
...
export function hydrateModel(config: DialogConfig): DialogModel {
  const state = canonicalToState(config.initialState, {
  ...
  const model: DialogModel = {
    ...INITIAL_MODEL,
    ...state,
    phase: primarySource(state).name ? "crop" : "upload",
...
export function dialogReducer(
  ...
  switch (action.type) {
    ...
    case "cancelReplace":
      if (!primarySource(model).name || model.phase !== "upload") {
        return model;
      }
      return {
        ...model,
    ...
    case "cropSubmitSucceeded": {
      ...
      return { ...next, phase: "complete", dirty: false };
    }
