...
export function openDialogWindow(prefix: string, cropdusterUrl: string): void {
  ...
  for (const name of ["image", "id"]) {
    data[name] = elementValue(
      document.getElementById(`id_${prefix}-0-${name}`),
    );
  }
  const thumbSelect = document.getElementById(`id_${prefix}-0-thumbs`);
  data.thumbs = elementValue(thumbSelect);
  if (thumbSelect instanceof HTMLSelectElement) {
    const tmpThumbs = [...thumbSelect.selectedOptions]
      .filter((option) => option.getAttribute("data-tmp-file") === "true")
      .map((option) => option.value)
      .filter(Boolean);
    data.tmp_thumbs = tmpThumbs.length ? tmpThumbs.join(",") : undefined;
  }
