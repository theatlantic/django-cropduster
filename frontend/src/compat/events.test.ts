...
describe("emitUpdate", () => {
  it("dispatches a native event when the page has no jQuery", () => {
    const native = vi.fn();
  ...
  it("dispatches once through each jQuery instance", () => {
  ...
  it("reaches a real jQuery handler once with positional arguments", async () => {
    ...
    expect(calls[0]?.slice(1)).toEqual(["lead_image", { crop: {} }]);
