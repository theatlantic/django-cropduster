 ...
 *
 * `cropduster:update` uses every jQuery instance when the page has one, which
 * preserves its positional `prefix` and `data` arguments. A page without
 * jQuery receives the same event name as a native CustomEvent.
 */

import { jqueryInstances, onAll, triggerOnAll } from "../dom/jquery";
...
/** Dispatch the completion event for `prefix` through the available channel. */
export function emitUpdate(prefix: string, data: unknown) {
  if (jqueryInstances().length) {
