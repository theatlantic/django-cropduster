...
import type { CropBox, Size } from "./geometry";
  ...
  it("defaultCropBox reproduces the stored crop box", () => {
    ...
    for (const vector of vectors) {
      ...
      compare(
        failures,
        `${label(vector)} box`,
        defaultCropBox(origW, origH, size),
        vector.select,
      );
    }
    assertNoFailures(failures);
...
describe("aspect band clamps the golden corpus does not reach", () => {
  ...
  it("raises the lower bound from max_h, then widens the default crop to it", () => {
    ...
    expect(defaultCropBox(1000, 1000, subject)).toEqual({
      x: 0,
      y: 250,
      w: 1000,
      h: 500,
  ...
  it("lowers the upper bound from max_w, then narrows the default crop to it", () => {
    ...
    expect(defaultCropBox(3000, 100, subject)).toEqual({
      x: 1400,
      y: 0,
      w: 200,
      h: 100,
  ...
  it("treats a min of exactly 1 as no minimum for the band only", () => {
    ...
    expect(defaultCropBox(3000, 100, sentinel)).toEqual({
      x: 0,
      y: 0,
      w: 3000,
      h: 100,
    ...
    expect(defaultCropBox(100, 3000, real)).toEqual({
      x: 0,
      y: 1475,
      w: 100,
      h: 50,
  ...
  it("leaves an inverted band alone when the ratio is fixed", () => {
    ...
    expect(defaultCropBox(674, 800, subject)).toEqual({
      x: 0,
      y: 131,
      w: 674,
      h: 539,
...
describe("fitToCrop", () => {
  ...
  const BANDED_BOUNDS = { w: 1000, h: 1000 };
  const BANDED_VECTORS: [string, Size, CropBox, CropBox][] = [
    [
      "derives the upper aspect from w and min_h",
      { name: "banded", w: 600, min_h: 400 },
      { x: 250, y: 375, w: 500, h: 250 },
      { x: 200, y: 300, w: 600, h: 400 },
    ],
    [
      "derives the lower aspect from w and max_h",
      { name: "banded", w: 600, max_h: 300 },
      { x: 250, y: 250, w: 500, h: 500 },
      { x: 146, y: 323, w: 707, h: 354 },
    ],
    [
      "derives the lower aspect from h and min_w",
      { name: "banded", h: 300, min_w: 600 },
      { x: 250, y: 250, w: 500, h: 500 },
      { x: 146, y: 323, w: 707, h: 354 },
    ],
    [
      "derives the upper aspect from h and max_w",
      { name: "banded", h: 300, max_w: 450 },
      { x: 250, y: 375, w: 500, h: 250 },
      { x: 275, y: 350, w: 450, h: 300 },
    ],
    [
      "preserves an inverted derived band",
      { name: "banded", w: 600, min_h: 400, max_h: 300 },
      { x: 250, y: 250, w: 500, h: 500 },
      { x: 100, y: 300, w: 800, h: 400 },
    ],
  ];

  for (const [label, size, reference, expected] of BANDED_VECTORS) {
    it(label, () => {
      expect(fitToCrop(size, reference, BANDED_BOUNDS)).toEqual(expected);
    });
  }
