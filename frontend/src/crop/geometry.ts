...
export interface Size {
  ...
  max_w?: SizeNumber;
  max_h?: SizeNumber;
  min_aspect?: SizeNumber;
  max_aspect?: SizeNumber;
  auto?: Size[] | null;
...
/**
 * Return the largest centered crop allowed by the size's fixed ratio or ratio
 * range.
 */
 ...
 * sizes normally omit `min_aspect`/`max_aspect`, so their values are derived
 * from the serialized dimensions in the same way as `Size.__init__()`.
 * Explicit aspect fields take precedence when present. Returns null for a
 * degenerate reference; the caller falls back to `defaultCropBox()`.
 */
export function fitToCrop(
  ...
  const derivedExtent = aspectExtent(size);
  const minAspect =
    size.min_aspect == null ? derivedExtent.min : toNumber(size.min_aspect);
  const maxAspect =
    size.max_aspect == null ? derivedExtent.max : toNumber(size.max_aspect);
