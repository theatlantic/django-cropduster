...
describe("canonicalToState", () => {
  ...
  it("matches the one standalone crop to the size, whatever it is named", () => {
    const existing = standaloneCrop();
    existing.thumbs["5e4aeac8f"]!.id = null;
    existing.thumbs["5e4aeac8f"]!.changed = false;
    const state = canonicalToState(existing, {
      standalone: true,
    ...
    expect(state.crops.crop?.changed).toBe(false);
  });
...
describe("the preview dimensions the legacy payload reports", () => {
  const cases: [
    ...
    [700, 500, [800, 500], 800, 500, "an original that fits inside the box"],
    [800, 500, [800, 500], 800, 500, "an original exactly the size of the box"],
    [1600, 1000, [800, 500], 800, 500, "an original scaled by a whole ratio"],
    [1300, 1016, [800, 500], 640, 500, "the recorded lead_image original"],
    [674, 800, [800, 500], 421, 500, "the recorded headshot original"],
    [
      null,
      null,
      [800, 500],
      800,
      500,
      "an image whose dimensions are not known",
    ],
    [700, 0, [800, 500], 800, 500, "an image with one dimension missing"],
    [
      1600,
      1000,
      [640, 360],
      576,
      360,
      "a scaled original under configured bounds",
    ],
    [
      300,
      200,
      ...
      640,
      360,
      "a small original under configured bounds",
    ],
    [null, null, [640, 360], 640, 360, "configured bounds with no dimensions"],
  ];

  for (const [width, height, bounds, w, h, name] of cases) {
