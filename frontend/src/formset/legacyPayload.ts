 ...
 *
 * A primary-key-less crop marked changed is a server suggestion, so retain
 * that pending state for the next crop request. A reopened standalone crop
 * also has no primary key, but its metadata describes an existing rendition.
 */
export function canonicalToState(
  ...
  for (const size of sizes) {
    ...
    crops[name] = {
      ...
      changed: Boolean(crop) && !thumb?.id && Boolean(thumb?.changed),
    };
  }
...
function legacyPreviewSize(
  ...
  const [boundsW, boundsH] = bounds;
  const dimensions = { preview_w: boundsW, preview_h: boundsH };
...
export function stateToLegacyComplete(
  ...
  legacyPreviewBounds: readonly [number, number] = DEFAULT_PREVIEW_SIZE,
  ...
  return {
    ...
    ...legacyPreviewSize(source, legacyPreviewBounds),
  };
}
