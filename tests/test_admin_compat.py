...
class TestAdminCompat(CropdusterTestCaseMediaMixin, AdminSelenosisTestCase):
    ...
    def test_update_event_keeps_its_jquery_signature(self):
        ...
        updates = self.js("return window.__updates;")
        self.assertEqual(len(updates), instances, updates)
        for call in updates:
            self.assertEqual(call['prefix'], 'headshot')
            self.assertEqual(call['image'], 41)
            self.assertEqual(call['type'], 'cropduster:update')
        self.assertEqual(self.js("return window.__native;"), [])
