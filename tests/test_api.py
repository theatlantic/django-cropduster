...
class TestState(ApiTestCase):
    ...
    def test_named_temporary_crop_keeps_its_session_filename(self):
        _article, image = self.article_image()
        main = image.thumbs.get(name='main')
        tmp_path = image.get_image_path('main', tmp=True)
        with image.storage.open(tmp_path, 'wb') as tmp_file:
            tmp_file.write(b'temporary crop')

        payload = self.assertPayload(self.get_state(
            id=image.pk, thumbs=str(main.pk), tmp_thumbs=str(main.pk)))

        self.assertTrue(payload['thumbs']['main']['tmp'])
        self.assertTrue(
            urlsplit(payload['thumbs']['main']['file_url']).path.endswith(
                'main_tmp.jpg'))

    def test_temporary_crop_claim_requires_its_file(self):
        _article, image = self.article_image()
        main = image.thumbs.get(name='main')
