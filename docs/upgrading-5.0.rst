...
``CropDuster.complete()`` no longer calls ``CropDuster.setThumbnails()``
    ...
    Listen for ``cropduster:update`` instead. When jQuery is present, the event
    retains its ``(event, prefix, data)`` signature on each jQuery instance.
    Without jQuery, Cropduster dispatches one native ``CustomEvent`` whose
    ``detail`` is ``{prefix, data}``.
