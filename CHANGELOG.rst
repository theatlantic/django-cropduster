...
* ``cropduster:update`` fires on every jQuery instance found on the page with
  its ``(event, prefix, data)`` signature. On a page without jQuery, it fires
  once as a native ``CustomEvent`` with ``detail = {prefix, data}``. The new
...
* The crop geometry is a line-for-line port of the 4.x dialog. Vitest compares
  ...
  rewrite and 13 explicit edge cases. The artifact records the SHA-256 digest
  of the 4.15.0 source.
